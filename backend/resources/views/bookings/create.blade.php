...
<div class="page-header">
    <div>
        <h1><i class="fas fa-plus-circle" style="color:#0e9ae0;margin-right:10px;"></i>New Booking</h1>
        <p>Create a hotel accommodation booking for a crew member</p>
    </div>
...
<div class="form-row">
    <div class="form-group">
        ...
        <select name="crew_id" id="crew_id" class="form-control {{ $errors->has('crew_id') ? 'is-invalid' : '' }}" required>
            <option value="">— Select Crew Member —</option>
            @foreach($crews as $crew)
                <option value="{{ $crew->id }}" {{ old('crew_id') == $crew->id ? 'selected' : '' }}>
                    {{ $crew->full_name }} ({{ $crew->nationality }})
                </option>
            @endforeach
        </select>
        ...
        @error('crew_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="form-group">
        <label class="form-label">Crew Title / Rank *</label>
        <input type="text" name="crew_title" class="form-control {{ $errors->has('crew_title') ? 'is-invalid' : '' }}"
            placeholder="e.g. Captain, Engineer, Cook, Deckhand"
            value="{{ old('crew_title') }}" required>
        @error('crew_title')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
</div>
...
<div class="form-row">
    ...
    <div class="form-group">
        ...
        <select name="hotel_id" id="hotel_id" class="form-control {{ $errors->has('hotel_id') ? 'is-invalid' : '' }}" required>
...
<div class="form-row">
    <div class="form-group">
        ...
        <input type="datetime-local" name="check_in" class="form-control {{ $errors->has('check_in') ? 'is-invalid' : '' }}"
    ...
    <div class="form-group">
        ...
        <input type="datetime-local" name="check_out" class="form-control {{ $errors->has('check_out') ? 'is-invalid' : '' }}"
...
<div class="form-row">
    <div class="form-group">
        <label class="form-label">Hotel Invoice Number</label>
        <input type="text" name="invoice_number" class="form-control"
            placeholder="e.g. INV-2026-001" value="{{ old('invoice_number') }}">
    </div>
    <div class="form-group">
...
<div class="form-group">
    ...
    <textarea name="remarks" class="form-control" placeholder="Any special notes, requirements, or remarks...">{{ old('remarks') }}</textarea>
</div>
    ...
    .then(data => {
        if (data.success) {
            ...
            if (selectId === 'crew_id') {
                ...
                option.text = `${data.crew.full_name}${data.crew.nationality ? ' ('+data.crew.nationality+')' : ''}`;
